...
# run scr   
    ...
    def time_thread(stop_event):
        ...
        while not stop_event.is_set():
            time_spent += 1
            stop_event.wait(1)  # Wait for 1 second before updating time_spent

    stop_event = threading.Event()
    time_spent = 0

    time_thread = threading.Thread(target=time_thread, args=(stop_event,))
    time_thread.start()
    ...
    while startVar == True:
        ...
        if x1 or y1 or x2 or y2:
            ...
            print(df["#"])
            max = int(df['#'].max())
            new_row = pd.DataFrame({'#':[max+1], 'length':[length], 'time':["time_spent"]})
            print(new_row)
